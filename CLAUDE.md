...
- **Reimbursements from AMZ** (missing from inbound, warehouse damage, warehouse lost) are a separate positive line item. They are NOT netted into refunds or fees. Note: **ReversalReimbursement** is deliberately categorized as Operational Fees, not Reimbursements, to match Elena's Sellerise-derived formula — see PNL_MAPPING.md "Operational Fees" section.
...
- **Posted-date bucketing is our convention**. Every event is attributed to its SP-API `PostedDate` (converted to PT-local for `daily_pnl.date`), never to its underlying order date. Elena's Sellerise spreadsheet buckets by order date, which produces a consistent ~2.5-2.85% drift on order-linked line items (Sales, Commission, FBAPerUnitFulfillmentFee) in Jan 2026 US — Amazon batches late-month settlements to post on Feb 1 UTC, so ~$5k of Jan orders land in our PT-Feb-1 bucket while Elena counts them in her Jan. This is a deliberate methodology difference: closing it would require per-event `AmazonOrderId` → order-date lookups against the Orders API. Deferred as future work; document the residual drift when comparing against Elena.
- **Warehouse-Lost reimbursements stay in `Reimbursements`.** Elena's Sellerise formula for Reimbursements excludes `WarehouseLost` (~$149 for Jan 2026 US), producing a small over-count on our side versus hers. We keep it in Reimbursements because SP-API's `AdjustmentType='WarehouseLost'` is semantically a customer-facing reimbursement (inventory Amazon lost), and Elena's exclusion appears to be a spreadsheet choice rather than an accounting principle. If Elena wants strict alignment she can subtract the WarehouseLost line manually; our stored total is the more complete view.
- **Settlement report ingestion is scoped to Taxes-remitted-to-Amazon only.** `GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE_V2` is a large report — every line item Amazon posted in the settlement period. All other rows in it (shipment charges, refunds, adjustments, service fees) are already captured by the daily by-date ETL and the monthly by-group reconciliation, so ingesting them again would double-count. The settlement ETL filters strictly to `amount-description` matching `MarketplaceFacilitatorTax*`, `MarketplaceFacilitatorVAT*`, or `TaxWithheld` and inserts those as `source='settlement'`, `category='selling_fees'`. If Amazon adds a new tax-remittance variant in the future, extend `TAXES_REMITTED_PREFIXES` in `amazon_settlement_etl.py`.
...
- **Months older than ~90 days can't be validated after the fact.** Amazon's Reports API `createdSince` parameter rejects any value more than 90 days old. Settlement reports for those months exist somewhere in Amazon but the list endpoint won't surface them. If you need older-month reconciliation, you'd need the individual `reportId`s (stashed from an earlier run) and fetch each directly — or accept the snapshot as of when the ETL last ran that month.
